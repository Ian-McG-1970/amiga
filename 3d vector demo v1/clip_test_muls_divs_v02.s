...
DEBUG_WRITE
			...
			move.l	#(6*40)+0,d1
			BSR		HEX32
			MOVE.L 	(CLIP_START_ROT_Y),D0
			move.l	#(6*40)+9,d1
			BSR		HEX32
			MOVE.L 	(CLIP_START_ROT_Z),D0
			move.l	#(6*40)+18,d1
			BSR		HEX32
			MOVE.L 	(CLIP_END_ROT_X),D0
			move.l	#(12*40)+0,d1
			BSR		HEX32
			MOVE.L 	(CLIP_END_ROT_Y),D0
			move.l	#(12*40)+9,d1
			BSR		HEX32
			MOVE.L 	(CLIP_END_ROT_Z),D0
			move.l	#(12*40)+18,d1
			BSR		HEX32
			MOVE.L 	(CLIPPED_ROT_X),D0
			move.l	#(18*40)+0,d1
			BSR		HEX32
			MOVE.L 	(CLIPPED_ROT_Y),D0
			move.l	#(18*40)+9,d1
			BSR		HEX32
			MOVE.L 	(CLIPPED_ROT_Z),D0
			move.l	#(18*40)+18,d1
			BSR		HEX32
...
.KEYT		MOVE.L	TST_ST_Z,D0
			SUBQ.L	#1,D0
			MOVE.L	D0,TST_ST_Z
			RTS
.KEYG		MOVE.L	TST_ST_Z,D0
			ADDQ.L	#1,D0
			MOVE.L	D0,TST_ST_Z
			RTS
.KEYF		MOVE.L	TST_ST_X,D0
			SUBQ.L	#1,D0
			MOVE.L	D0,TST_ST_X
			RTS
.KEYH		MOVE.L	TST_ST_X,D0
			ADDQ.L	#1,D0
			MOVE.L	D0,TST_ST_X
			RTS

.KEYI		MOVE.L	TST_EN_Z,D0
			SUBQ.L	#1,D0
			MOVE.L	D0,TST_EN_Z
			RTS
.KEYK		MOVE.L	TST_EN_Z,D0
			ADDQ.L	#1,D0
			MOVE.L	D0,TST_EN_Z
			RTS
.KEYJ		MOVE.L	TST_EN_X,D0
			SUBQ.L	#1,D0
			MOVE.L	D0,TST_EN_X
			RTS
.KEYL		MOVE.L	TST_EN_X,D0
			ADDQ.L	#1,D0
			MOVE.L	D0,TST_EN_X
			RTS

CLIP_LINE_TEST	MOVEQ.L	#CC_ON,D6 			; and clip code
				...
				BPL.S	.TST_FRONT_ST
					...
					BRA.S	.TST_BEHIND_ST
.TST_FRONT_ST:	BSR		CLIPCODE			; calc clip code
.TST_BEHIND_ST:	MOVE.B	D4,(TST_ST_CC)
				...
				BPL.S	.TST_FRONT_EN
					MOVEQ.L	#CC_BEHIND,D4	;  set clip code to behind
					BRA.S	.TST_BEHIND_EN
.TST_FRONT_EN:	BSR		CLIPCODE			; calc clip code
.TST_BEHIND_EN:	MOVE.B	D4,(TST_EN_CC)
				AND.B	D4,D5					; object and clip code
				OR.B	D4,D6					; object or clip code

				MOVE.B	D5,(TST_AND_CC)
				MOVE.B	D6,(TST_OR_CC)

				BSR DEBUG_CLIP_TEST_WRITE

;	Line(XYZ(start.x+width, 0, height-start.z), XYZ(end.x+width, 0, height-end.z), 40);
				MOVE.L	(TST_ST_Z),D0
				ADD.L	#SCREEN_HOR_MIDDLE,D0
				MOVE.L	(TST_ST_Z),D1
				NEG.L	D1
				ADD.L	#SCREEN_VER_MIDDLE,D1
				MOVE.L	(TST_EN_Z),D2
				ADD.L	#SCREEN_HOR_MIDDLE,D2
				MOVE.L	(TST_EN_Z),D3
				NEG.L	D3
				ADD.L	#SCREEN_VER_MIDDLE,D3
				BSR		LINE_DRAW
...
.TST_ST_OFF_EN_ON:	
						...
						BSR CLIPSHIFT_TEST_MD
						MOVE.L 	D0,(CLIPPED_ROT_X)
						MOVE.L 	D2,(CLIPPED_ROT_Z)

						MOVE.L 	(CLIPPED_ROT_X),D0
						ADD.L	#SCREEN_HOR_MIDDLE,D0
						MOVE.L 	(CLIPPED_ROT_Z),D1
						NEG.L	D1
						ADD.L	#SCREEN_VER_MIDDLE,D1
						MOVE.L	(TST_EN_X),D2
						ADD.L	#SCREEN_HOR_MIDDLE,D2
...
.TST_NO_CLIP:	MOVE.L	(TST_ST_X),D0
				ADD.L	#SCREEN_HOR_MIDDLE,D0
				MOVE.L	(TST_ST_Z),D1
				NEG.L	D1
				ADD.L	#SCREEN_VER_MIDDLE,D1
...
.PNT_FRONT:		BSR		CLIPCODE			; calc clip code
				...
				MOVE.L	D5,CC_AND_TEST
				MOVE.L	D6,CC_OR_TEST
...
;							BRA		.DRAW_LINE_NEXT
					...
					MOVE.W 	D5,(CLIP_START_POINT)
					MOVE.W 	D6,(CLIP_END_POINT)
	...
	MOVE.W #$0110,$DFF180
						...
						BEQ	.CLIP_RIGHT_START		; no - bit is clear

.CLIP_RIGHT_END:				; end point is off right
					MOVE.L	(CLIP_START_ROT_X),D0
	...
	BSR CLIPSHIFT
							MOVE.L 	D0,(CLIPPED_ROT_X)
							MOVE.L 	D1,(CLIPPED_ROT_Y)
							MOVE.L 	D2,(CLIPPED_ROT_Z)

							MOVE.L 	(CLIPPED_ROT_X),D0
							MOVE.L 	(CLIPPED_ROT_Y),D1
							MOVE.L 	(CLIPPED_ROT_Z),D2
							BSR		PERSPECTIVE
...
.CLIP_RIGHT_START:				; start point is off right
							BRA		.DRAW_LINE_NEXT
					MOVE.L	(CLIP_END_ROT_X),D0
					MOVE.L 	(CLIP_END_ROT_Y),D1
					MOVE.L 	(CLIP_END_ROT_Z),D2
					MOVE.L	(CLIP_START_ROT_X),D3
...
.CLIP_LEFT_TST:		BTST	#CC_LEFT_BIT,D2    ; Check if either off left
					BEQ.S 	.CLIP_BOTTOM_TST

						BTST	#CC_LEFT_BIT,D0  	; Check if start off left
						BEQ.S	.CLIP_LEFT_START

.CLIP_LEFT_END:					; end point is off left
							BRA		.DRAW_LINE_NEXT

.CLIP_LEFT_START:				; start point is off left
							BRA		.DRAW_LINE_NEXT	

.CLIP_BOTTOM_TST:	BTST	#CC_BOTTOM_BIT,D2    ; Check if either off bottom
					BEQ.S	.CLIP_TOP_TST

						BTST	#CC_BOTTOM_BIT,D0 	; Check if start off bottom
						BEQ.S	.CLIP_BOTTOM_START

.CLIP_BOTTOM_END:				; end point is off bottom
							BRA		.DRAW_LINE_NEXT

.CLIP_BOTTOM_START:				; start point is off bottom
							BRA		.DRAW_LINE_NEXT	

.CLIP_TOP_TST:		BTST	#CC_TOP_BIT,D2    ; Check if either off top
					BEQ.S	.CLIP_EXIT

						BTST	#CC_TOP_BIT,D0    	; Check if start off top
.CLIP_TOP_END:					; end point is off top
							BRA		.DRAW_LINE_NEXT

.CLIP_TOP_START:				; start point is off top
							BRA		.DRAW_LINE_NEXT	

.CLIP_EXIT:	BRA		.DRAW_LINE_NEXT	

CLIP_RIGHT_START_POINT	MOVE.L 	(CLIP_START_ROT_X),D0
						SUB.L 	(CLIP_END_ROT_X),D0	; const int D_x = Start.x - End.x;

						MOVE.L 	(CLIP_START_ROT_Y),D1
						SUB.L 	(CLIP_END_ROT_Y),D1	; const int D_y = Start.y - End.y;

						MOVE.L 	(CLIP_START_ROT_Z),D2
						SUB.L 	(CLIP_END_ROT_Z),D2	; const int D_z = Start.z - End.z;

					MOVE.L 	D2,D4					; const int D_Diff = D_z - D_x
					SUB.L 	D0,D4

					MOVE.L 	(CLIP_END_ROT_X),D3	; const int S_Diff = Start.x - Start.z;
					SUB.L 	(CLIP_END_ROT_Z),D3

					lsl.l 	#7,d3
					lsl.l 	#8,d3
					divs	d4,d3					;	const int Scale = S_Diff *32768 / D_Diff;

					MULS	D3,D0
					asr.l #7,d0
					asr.l #8,d0
					ADD.L 	(CLIP_END_ROT_X),D0	; + Start.x

					MULS	D3,D1
					asr.l #7,d1
					asr.l #8,d1
					ADD.L 	(CLIP_END_ROT_Y),D1	; + Start.y

					MULS	D3,D2
					asr.l #7,d2
					asr.l #8,d2
					ADD.L 	(CLIP_END_ROT_Z),D2	; + Start.z

					RTS

CLIP_RIGHT_END_POINT		MOVE.L 	(CLIP_END_ROT_X),D0
					SUB.L 	(CLIP_START_ROT_X),D0	; const int D_x = End.x - Start.x

					MOVE.L 	(CLIP_END_ROT_Y),D1
					SUB.L 	(CLIP_START_ROT_Y),D1	; const int D_y = End.y - Start.y

					MOVE.L 	(CLIP_END_ROT_Z),D2
					SUB.L 	(CLIP_START_ROT_Z),D2	; const int D_z = End.z - Start.z

					MOVE.L 	D2,D4					; const int D_Diff = D_z - D_x
					SUB.L 	D0,D4

					MOVE.L 	(CLIP_START_ROT_X),D3	; const int S_Diff = Start.x - Start.z;
					SUB.L 	(CLIP_START_ROT_Z),D3
					...
					asr.l #7,d0
					asr.l #8,d0
					ADD.L 	(CLIP_START_ROT_X),D0	; + Start.x

					MULS	D3,D1
					asr.l #7,d1
					asr.l #8,d1
					ADD.L 	(CLIP_START_ROT_Y),D1	; + Start.y

					MULS	D3,D2
					asr.l #7,d2
					asr.l #8,d2
					ADD.L 	(CLIP_START_ROT_Z),D2	; + Start.z
				RTS

CLIP_RIGHT_START_POINT_V1	MOVE.L 	(CLIP_START_ROT_X),D0
						SUB.L 	(CLIP_END_ROT_X),D0	; const int D_x = Start.x - End.x;
					...
					MULS 	D3,D0					; D_x * S_Diff
					DIVS	D4,D0					; / D_Diff
					EXT.L	D0
					ADD.L 	(CLIP_END_ROT_X),D0	; + Start.x

					MULS 	D3,D1					; D_y * S_Diff
					DIVS	D4,D1					; / D_Diff
					EXT.L	D1
					ADD.L 	(CLIP_END_ROT_Y),D1	; + Start.y

					MULS 	D3,D2					; D_z * S_Diff
					DIVS	D4,D2					; / D_Diff
					EXT.L	D2
					ADD.L 	(CLIP_END_ROT_Z),D2	; + Start.z
...
CLIP_RIGHT_END_POINT_V1		MOVE.L 	(CLIP_END_ROT_X),D0
					SUB.L 	(CLIP_START_ROT_X),D0	; const int D_x = End.x - Start.x
					MOVE.L 	D0,(TEST0)
...
CLIP_SHIFT_VAL	EQU 65536*8192		;	const int clipshiftval=65536*16384;

CLIPSHIFT_TEST_MD
	MOVE.L 	D3,D6
	SUB.L 	D0,D6	; dx = Bx - Ax

	MOVE.L 	D5,D7
	SUB.L 	D2,D7	; dz = Bz - Az
...
;	const int D_Diff = D_z - D_x;
;	const int S_Diff = Start.x - Start.z;	
;	xyz rc;
;	rc.x = Start.x + ( (D_x * S_Diff) / D_Diff );
;	rc.z = Start.z + ( (D_z * S_Diff) / D_Diff );
;	return rc;
;}

;const xyz Clip3DRightWorks(const xyz Start, const xyz End)
;{
;	const int D_x = End.x - Start.x;
;	const int D_z = End.z - Start.z;
...
CLIPSHIFT_TEST
	MOVE.L	#CLIP_SHIFT_VAL,D6

			ADD.L	D6,D0		;	start.x+=clipshiftval;
			ADD.L	D6,D2		;	start.z+=clipshiftval;
			ADD.L	D6,D3		;	end.x+=clipshiftval;
			ADD.L	D6,D5		;	end.z+=clipshiftval;
			...
			BCC.S 	.Z_NOSWAP
				EXG.L	D0,D3	; swap x
				EXG.L	D1,D4	; swap y
				EXG.L	D2,D5	; swap highest and lowest z
.Z_NOSWAP:			
			MOVE.L 	D2,(STARTZ)
			MOVE.L 	D5,(ENDZ)

			SUB.L	D2,D5		;	int edz=end.z-start.z;	// zdifference=high point
			MOVE.L 	D5,(EDZ)
			MOVE.L 	D5,(EDZ_BK)
...
.X_NEG
			...
			SUB.L	D0,D3		;	int edx=end.x-start.x
			MOVE.L 	D3,(EDX)
			MOVE.L 	D3,(EDX_BK)

			MOVEQ.L	#0,D3				;	int sdx=0;		// low point
			MOVE.L 	D3,(SDX)
			MOVE.L 	D3,(SDZ)
...
.LOOP_XNEG	
	...
	SUBQ.L	#1,D3
	BEQ 	.EXIT_XNEG
	MOVE.L 	D3,(TEMP_CNT_BK)
	
				MOVE.L 	(SDX),D6
				ADD.L 	(EDX),D6
				ASR.L 	#1,D6			;		middle.x=((sdx+edx)/2);//+1;
				MOVE.L 	D6,(MX)

				MOVE.L 	(SDZ),D5
				ADD.L 	(EDZ),D5
				ASR.L 	#1,D5			;		middle.z=((sdz+edz)/2);//+1;
				MOVE.L 	D5,(MZ)

				ADD.L 	(STARTX),D6		;		const int tx=middle.x+start.x;
				ADD.L 	(STARTZ),D5		;		const int tz=middle.z+start.z;

				CMP.L 	D5,D6			;		if (tx==tz) 
				BEQ 	.EXIT_XNEG		;			break;
				BCS.S 	.HIGHER_XNEG	;		if (tx<tz)
					MOVE.L (MX),D5
					MOVE.L D5,(SDX)		;			sdx=middle.x;
					MOVE.L (MZ),D6
					MOVE.L D6,(SDZ)		;			sdz=middle.z;
					BRA 	.LOOP_XNEG	;		else

.HIGHER_XNEG		MOVE.L (MX),D5
					MOVE.L D5,(EDX)		;			edx=middle.x;
					MOVE.L (MZ),D6
					MOVE.L D6,(EDZ)		;			edz=middle.z;
					BRA		.LOOP_XNEG
...
.X_POS
			...
			SUB.L	D3,D0		;	int edx=end.x-start.x
			MOVE.L 	D0,(EDX)
			MOVE.L 	D0,(EDX_BK)
...
.LOOP_XPOS
	...
	MOVE.L 	D3,(TEMP_CNT_BK)
				...
				CMP.L 	D5,D6			;		if (tx==tz) 
				BEQ.S 	.EXIT_XPOS			;			break;
				BCS.S 	.HIGHER_XPOS	;		if (tx<tz)
					MOVE.L (MX),D5
					MOVE.L D5,(SDX)		;			sdx=middle.x;
					MOVE.L (MZ),D6
					MOVE.L D6,(SDZ)		;			sdz=middle.z;
...
.EXIT_XNEG
				MOVE.L	(MX),D0
				ADD.L 	(STARTX),D0			;	middle.x+=start.x;
				MOVE.L 	(MZ),D2
				ADD.L 	(STARTZ),D2			;	middle.z+=start.z;

				MOVE.L	#CLIP_SHIFT_VAL,D6
				SUB.L 	D6,D0				;	middle.x-=clipshiftval;
				SUB.L 	D6,D1				;	middle.y-=clipshiftval;
				SUB.L 	D6,D2				;	middle.z-=clipshiftval;

			RTS

.EXIT_XPOS
				MOVE.L 	(STARTX),D0			;	middle.x+=start.x;
				SUB.L	(MX),D0
				MOVE.L 	(STARTZ),D2			;	middle.z+=start.z;
				SUB.L 	(MZ),D2
...
;			SUBQ.L 	#1,D2
			...
			MOVE.L	D3,D7
			SUB.L 	D0,D7		;	int edx=end.x-start.x;	// xdifference=high point
			BPL.S	.CONT		;	if (edx<0)
			
				NEG.L	D7			;		edx=-edx;
				MOVE.L	D3,D0		;		start.x=end.x;
.CONT
;			SUBQ.L 	#1,D0
			MOVE.L 	D7,(EDX)
			MOVE.L 	D0,(STARTX)
			...
			MOVEQ.L	#0,D3				;	int sdx=0;		// low point
			MOVE.L	D3,D4				;	int sdz=0;		// low point
			MOVE.L 	D3,(SDX)
			MOVE.L 	D4,(SDZ)
...
;				addq.l #1,d5
				...
				MOVE.L 	(SDY),D4
				ADD.L 	(EDY),D4
				ASR.L 	#1,D4			;		middle.y=((sdx+edx)/2);//+1;
;				addq.l #1,d4
				MOVE.L 	D4,(MY)
				...
				CMP.L 	D5,D6			;		if (tx==tz) 
				BEQ.S 	.EXIT			;			break;
				BCS.S 	.HIGHER			;		if (tx<tz)
					MOVE.L (MX),D5
					MOVE.L D5,(SDX)		;			sdx=middle.x;
					MOVE.L (MY),D4
					MOVE.L D4,(SDY)		;			sdx=middle.x;
					MOVE.L (MZ),D6
					MOVE.L D6,(SDZ)		;			sdz=middle.z;
					BRA 	.LOOP		;		else
...
.EXIT
				...
				ADD.L 	(STARTX),D0			;	middle.x+=start.x;
				MOVE.L 	(MY),D1
				ADD.L 	(STARTY),D1			;	middle.y+=start.y;
				MOVE.L 	(MZ),D2
				ADD.L 	(STARTZ),D2			;	middle.z+=start.z;
...
CC_TEST			DC.L $12345678
CC_AND_TEST		DC.L $12345678
CC_OR_TEST		DC.L $12345678
...
CLIP_START_POINT	DS.W	1
CLIP_END_POINT		DS.W	1
