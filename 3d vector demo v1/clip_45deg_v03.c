...
//xz start={150, 30}; //{0x0fc20,0x0fa7d};
//xz end={20,100}; //{0x0f20,0x07683};
xz start={0x0fc20,0x0fa7d};
xz end={0x0f20,0x07683};
...
const xz MidPoint(const xz start, const xz end)
{
	return XZ((start.x+end.x)/2,(start.z+end.z)/2);
}

const void Line(const xz start, const xz end, const unsigned char colour)
{
	const xz half=MidPoint(start,end);
	const xz one_quarter=MidPoint(start,half);
	const xz three_quarters=MidPoint(end,half);
...
const xz Clip3DRightWorks(const xz Start, const xz End)
{
	const int D_x = End.x - Start.x;
	const int D_z = End.z - Start.z;
...
//	Line(XZ(start.x+width, height-start.z), XZ(end.x+width, height-end.z), 40);
//	Line(XZ(start.z+width, height-start.z), XZ(end.z+width, height-end.z), 35);
	
//	BigPlot(XZ(start.x+width, height-start.z), 30);
//  	BigPlot(XZ(end.x+width, height-end.z), 20);

	const xz clip=Clip3DRightWorks(start, end);
//  	BigPlot(XZ(clip.x+width, height-clip.z), 40);
  ...
  sprintf(string,"s %4i %4i e %4i %4i c %4i %4i\n",start.x,start.z,end.x,end.z,clip.x,clip.z); TextOutA(hdcScreen,1,1,string, strlen(string));
