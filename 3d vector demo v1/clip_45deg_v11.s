...
.CLIP_RIGHT_END:	MOVE.L	(CLIP_START_ROT_X),D3	; end point is off right
...
.CLIP_RIGHT_START:	MOVE.L	(CLIP_START_ROT_X),D0	; start point is off right
...
.CLIP_LEFT_TST:		BTST	#CC_LEFT_BIT,D2    ; Check if either off right
					BEQ 	.CLIP_BOTTOM_TST		; no - bit is clear

						BTST 	#CC_LEFT_BIT,D0  	; Check if start off right
						BEQ	.CLIP_LEFT_START		; no - bit is clear

.CLIP_LEFT_END:		MOVE.L	(CLIP_START_ROT_X),D3	; end point is off right
					MOVE.L 	(CLIP_START_ROT_Y),D4
					MOVE.L 	(CLIP_START_ROT_Z),D5
					MOVE.L	(CLIP_END_ROT_X),D0
					MOVE.L 	(CLIP_END_ROT_Y),D1
...
.CLIP_LEFT_START:	MOVE.L	(CLIP_START_ROT_X),D0	; start point is off right
					MOVE.L 	(CLIP_START_ROT_Y),D1
					MOVE.L 	(CLIP_START_ROT_Z),D2
					MOVE.L	(CLIP_END_ROT_X),D3
					MOVE.L 	(CLIP_END_ROT_Y),D4
...
.CLIP_BOTTOM_TST:	BTST	#CC_BOTTOM_BIT,D2    ; Check if either off right
...
.CLIP_TOP_TST:		BTST	#CC_TOP_BIT,D2    ; Check if either off right
					BEQ 	.CLIP_EXIT			; no - bit is clear
