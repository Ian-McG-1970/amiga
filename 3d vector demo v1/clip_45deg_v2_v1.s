...
.POINT_LOOP:	MOVEM.W (A0)+,D0/D1/D2		; get point
				...
				BPL.S	.PNT_FRONT
					...
					MOVEQ.L	#0,D4			;  set clip code to behind
...
.CLIP_LINE:						; D0/D1 = start/end clip code D2=OR clip codes
					...
					BTST	#CC_RIGHT_BIT,D2    ; Check if either off right		; if ( (cc & CC_OFF_RIGHT) NE CC_ALL_ON)
					BEQ 	.CLIP_LEFT_TST		; no - bit is clear

						BTST 	#CC_RIGHT_BIT,D0  	; Check if start off right	; if ( (start_cc & CC_OFF_RIGHT) NE CC_ALL_ON)
						BEQ		.CLIP_RIGHT_START	; no so end is off right

.CLIP_RIGHT_END:	MOVE.L	(CLIP_START_ROT_X),D0	; yes so start is off right
					...
					BNE 	.CLIP_RIGHT_CONT
						...
						MOVE.W	D1,(CLIP_START_PERS_H)
						BRA 	.CLIP_RIGHT_CONT

.CLIP_RIGHT_START:	MOVE.L	(CLIP_START_ROT_X),D3	; start point is off right
...
.CLIP_RIGHT_CONT:	MOVE.B	(CLIP_START_CC),D4
					AND.B 	(CLIP_END_CC),D4
;					tst.b d4
					BNE		.DRAW_LINE_NEXT			; if ( (start_cc & end_cc) NE CC_ALL_ON)

					MOVE.B 	(CLIP_START_CC),D4
					OR.B 	(CLIP_END_CC),D4
;					tst.b d4
					BNE		.CLIP_LEFT_TST			; if ( (start_cc | end_cc) EQ CC_ALL_ON)

							MOVE.W	(CLIP_START_PERS_H),D0
							MOVE.W 	(CLIP_START_PERS_V),D1
...
.CLIP_LEFT_CONT		MOVE.B	(CLIP_START_CC),D4
					AND.B 	(CLIP_END_CC),D4
;					tst.b d4
					BNE		.DRAW_LINE_NEXT			; if ( (start_cc & end_cc) NE CC_ALL_ON)
					...
					OR.B 	(CLIP_END_CC),D4
;					tst.b d4
					BNE		.CLIP_BOTTOM_TST		; if ( (start_cc | end_cc) EQ CC_ALL_ON)
...
.CLIP_BOTTOM_CONT:	MOVE.B	(CLIP_START_CC),D4
					AND.B 	(CLIP_END_CC),D4
;					tst.b d4
					BNE		.DRAW_LINE_NEXT			; if ( (start_cc & end_cc) NE CC_ALL_ON)
					...
					OR.B 	(CLIP_END_CC),D4
;					tst.b d4
					BNE		.CLIP_TOP_TST		; if ( (start_cc | end_cc) EQ CC_ALL_ON)
...
.CLIP_TOP_CONT:		MOVE.B	(CLIP_START_CC),D4
					AND.B 	(CLIP_END_CC),D4
;					tst.b d4
					BNE		.DRAW_LINE_NEXT			; if ( (start_cc & end_cc) NE CC_ALL_ON)
					...
					OR.B 	(CLIP_END_CC),D4
;					tst.b d4
					BNE		.DRAW_LINE_NEXT		; if ( (start_cc | end_cc) EQ CC_ALL_ON)
...
;			MOVEQ.L	#CC_ON,D4
.B_XTEST	TST.L	D0						; X IS +VE
			BPL.S	.B_RIGHT				; YES SO OFF RIGHT

.B_LEFT		MOVEQ.L	#CC_LEFT,D4			; OFF
			BRA.S	.B_YTEST

.B_RIGHT	MOVEQ.L	#CC_RIGHT,D4	; OFF
			
.B_YTEST	TST.L	D1					; Y IS +VE
			BPL.S	.B_BOTTOM			; YES SO OFF BOTTOM

.B_TOP		OR.B	#CC_TOP,D4			; OFF
			RTS

.B_BOTTOM	OR.B	#CC_BOTTOM,D4		; OFF
			RTS
